...
# Set logger
LOGGER = set_logging(LOGGING_NAME, verbose=VERBOSE)  # define globally (used in 1_train_det.py, 1_val_det.py, 1_val_det.py, etc.)
...
def yaml_save(file="data.yaml", data=None, header=""):
    ...
    Args:
        ...
        data (dict): Data to save in tool format.
        header (str, optional): tool header to add.
    ...
    # Dump data to file in tool format
...
def yaml_load(file="data.yaml", append_filename=False):
    ...
    Args:
        file (str, optional): File name. Default is 'data.yaml'.
        append_filename (bool): Add the tool filename to the tool dictionary. Default is False.

    Returns:
        (dict): tool data and file name.
    """
    assert Path(file).suffix in {".yaml", ".yml"}, f"Attempting to load non-tool file {file} with yaml_load()"
    with open(file, errors="ignore", encoding="utf-8") as f:
        ...
        # Add tool filename to dict and return
...
def yaml_print(yaml_file: Union[str, Path, dict]) -> None:
    """
    Pretty prints a tool file or a tool-formatted dictionary.

    Args:
        yaml_file: The file path of the tool file or a tool-formatted dictionary.
...
def set_sentry():
    ...
    sync=True in settings. Run 'yolo settings' to see and update settings tool file.
...
class SettingsManager(dict):
    """
    Manages Ultralytics settings stored in a tool file.

    Args:
        file (str | Path): Path to the Ultralytics settings tool file. Default is USER_CONFIG_DIR / 'settings.yaml'.
        version (str): Settings version. In case of local version mismatch, new default settings will be saved.
    """

    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        """Initialize the SettingsManager with default settings, load and validate current settings from the tool
        file.
        """
        import copy
        import hashlib
    ...
    def load(self):
        """Loads settings from the tool file."""
        super().update(yaml_load(self.file))

    def save(self):
        """Saves the current settings to the tool file."""
